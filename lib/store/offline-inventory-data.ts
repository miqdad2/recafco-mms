...
// from `store.issue` (which also drives Materials-Request issuance, Job Card
// visibility scoping, and Inventory Check). Maintenance Data Entry runs the
// department's daily opening-stock/receive/issue workflow and must not be
// read-only here, but granting them `store.issue` outright would silently
// hand them those unrelated capabilities too. Store Keeper / Maintenance
// Manager / Super Admin keep working exactly as before via `store.issue`.
