...
- `prisma/migrations` is the active schema history. Historical SQL migrations that predate the Prisma migration history are archived under `docs/archive/supabase-migrations/` for reference only.
- `components/layout`, `components/dashboard`, and `components/ui` contain reusable enterprise UI building blocks.

## Phase 2 Implementation Plan

- `assets`, `asset_documents`, `parts`, `work_orders`, `work_order_labor`, `work_order_materials`, `work_order_status_history`, and `work_order_attachments` are added in the PostgreSQL schema.
